...
echo ✓ Redis đã khởi động
echo.

echo [2/9] Khởi động MongoDB...
docker run -d ^
  --name mongo ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
...
echo [3/9] Khởi động MinIO (Object Storage)...
docker run -d ^
  --name minio ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
...
echo ✓ MinIO đã khởi động
echo.

echo Đợi 5 giây để database khởi động hoàn toàn...
timeout /t 5 /nobreak >nul
echo.

echo [4/9] Khởi động OpenVidu Server (Core Engine)...
docker run -d ^
  --name openvidu ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
  ...
  -v "%CURRENT_DIR%livekit.yaml:/etc/livekit.yaml:ro" ^
  -v "%CURRENT_DIR%scripts\entrypoint_openvidu.sh:/scripts/entrypoint.sh:ro" ^
  -e LAN_MODE=%LAN_MODE% ^
  -e LAN_PRIVATE_IP=%LAN_PRIVATE_IP% ^
  --entrypoint /bin/sh ^
  docker.io/openvidu/openvidu-server:3.5.0 ^
  /scripts/entrypoint.sh --config /etc/livekit.yaml
echo ✓ OpenVidu Server đã khởi động
echo.

echo [5/9] Khởi động Dashboard...
docker run -d ^
  --name dashboard ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
...
echo ✓ Dashboard đã khởi động
echo.

echo [6/9] Khởi động Operator...
docker run -d ^
  --name operator ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
...
echo ✓ Operator đã khởi động
echo.

echo [7/9] Khởi động Caddy Proxy (Gateway)...
docker run -d ^
  --name caddy-proxy ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
  ...
  -p 7880:7880 ^
  -p 9443:9443 ^
  -v "%CURRENT_DIR%custom-layout:/var/www/custom-layout:ro" ^
...
echo ✓ Caddy Proxy đã khởi động
echo.

echo [8/9] Khởi động OpenVidu Meet (Web App)...
docker run -d ^
  --name openvidu-meet ^
  --network %NETWORK_NAME% ^
  --restart on-failure ^
  --add-host host.docker.internal:host-gateway ^
  -p 9080:6080 ^
  -v "%CURRENT_DIR%scripts\entrypoint_openvidu_meet.sh:/scripts/entrypoint.sh:ro" ^
  -v "%CURRENT_DIR%scripts\utils.sh:/scripts/utils.sh:ro" ^
  -e USE_HTTPS=%USE_HTTPS% ^
  -e LAN_MODE=%LAN_MODE% ^
  -e LAN_DOMAIN=%LAN_DOMAIN% ^
  -e LAN_PRIVATE_IP=%LAN_PRIVATE_IP% ^
...
echo ✓ OpenVidu Meet đã khởi động
echo.

echo [9/9] Khởi động Ingress ^& Egress (Streaming/Recording)...
docker run -d ^
  --name ingress ^
  --network %NETWORK_NAME% ^
  --restart unless-stopped ^
  ...
  -p 8085:8085 ^
  -p 7895:7895/udp ^
  -v "%CURRENT_DIR%ingress.yaml:/etc/ingress.yaml:ro" ^
