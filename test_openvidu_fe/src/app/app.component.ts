...
export class AppComponent implements OnDestroy {
    ...
    getParticipantMicStatus(identity: string): boolean {
        // Local
        if (this.room()?.localParticipant.identity === identity) return this.isMicOn;
        // Remote
        const tracks = Array.from(this.remoteTracksMap().values());
        const micTrack = tracks.find((t) => t.participantIdentity === identity && t.trackPublication.kind === 'audio');
        return micTrack ? !micTrack.trackPublication.isMuted : false;
    }

    getParticipantCamStatus(identity: string): boolean {
        // Local
        if (this.room()?.localParticipant.identity === identity) return this.isCameraOn;
        // Remote
        const tracks = Array.from(this.remoteTracksMap().values());
    ...
    async joinRoom() {
        ...
        // Theo dõi trạng thái Mute/Unmute để cập nhật UI
        room.on(RoomEvent.TrackMuted, (pub, p) => {
            console.log('TrackMuted observed:', p.identity, pub.kind, 'isLocal:', p.isLocal);

            // Ép Signal cập nhật Map để UI lặp lại phương thức getParticipantMicStatus
            this.remoteTracksMap.update((prev) => new Map(prev));

            // Nếu là chính mình, đồng bộ cờ local
            if (p.isLocal) {
                if (pub.kind === 'audio') this.isMicOn = false;
                if (pub.kind === 'video') this.isCameraOn = false;
            }
        });

        room.on(RoomEvent.TrackUnmuted, (pub, p) => {
            console.log('TrackUnmuted observed:', p.identity, pub.kind, 'isLocal:', p.isLocal);

            // Ép Signal cập nhật Map
            this.remoteTracksMap.update((prev) => new Map(prev));

            // Nếu là chính mình, đồng bộ cờ local
            if (p.isLocal) {
                if (pub.kind === 'audio') this.isMicOn = true;
                if (pub.kind === 'video') this.isCameraOn = true;
            }
        });
        ...
        try {
            ...
            this.participants.set([localId, ...existing]);
    ...
    async toggleMic() {
        ...
        try {
            // Sử dụng setMicrophoneEnabled để LiveKit tự động xử lý và gửi signal mute/unmute đến mọi người
            ...
            this.addToast('Không thể thay đổi trạng thái micro', 'error');
        }
    }
    ...
    async toggleCamera() {
        ...
        try {
            ...
            this.addToast('Không thể thay đổi trạng thái camera', 'error');
        }
    }
